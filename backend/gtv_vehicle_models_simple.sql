-- Simple GTV Motor Vehicle Models Setup
-- This script creates vehicle models without using information_schema

-- Step 1: Create vehicle_models table structure
CREATE TABLE IF NOT EXISTS `vehicle_models` (
  ...
  `color_options` text DEFAULT NULL COMMENT 'Available colors as JSON array',
  `year_range` varchar(20) DEFAULT NULL COMMENT 'Model year range (e.g., 2020-2025)',
  `specifications` text DEFAULT NULL COMMENT 'Technical specifications as JSON',
...
INSERT INTO `vehicle_models` (
  ...
  '2020-2025',
  '{"dimensions": "4,400 × 1,831 × 1,653 mm", "seating_capacity": "5 seats", "segment": "Compact Entry SUV", "features": "Modern design, safety features, SUV styled for modernity", "engine": "1.5L petrol with CVT transmission", "price_usd": 19999, "weight": "Approx. 1,400 kg", "fuel_capacity": "50L", "ground_clearance": "180mm"}',
  1
...
-- Step 7: Add vehicle_model_id column to vehicles table (if it doesn't exist)
-- Note: This will fail silently if column already exists
ALTER TABLE `vehicles` ADD COLUMN `vehicle_model_id` int(11) DEFAULT NULL AFTER `model`;

-- Step 8: Add index for vehicle_model_id (if it doesn't exist)
-- Note: This will fail silently if index already exists
ALTER TABLE `vehicles` ADD INDEX `idx_vehicle_model_id` (`vehicle_model_id`);

-- Step 9: Remove KESSOR model (not in the provided specifications)
DELETE FROM `vehicle_models` WHERE `name` = 'KESSOR';

-- Step 10: Show final vehicle models overview
SELECT
...
-- Step 11: Show price summary by category
SELECT
...
-- Step 12: Show current vehicles (if any)
SELECT
  'CURRENT VEHICLES (first 10):' as title;
