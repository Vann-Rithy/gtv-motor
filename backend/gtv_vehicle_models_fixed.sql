-- Complete GTV Motor Vehicle Models with Full Specifications (Fixed)
...
-- Step 8: Update existing vehicles to reference vehicle_models
UPDATE `vehicles` v
JOIN `vehicle_models` vm ON v.model = vm.name
SET v.vehicle_model_id = vm.id
WHERE v.model IN ('SOBEN', 'KAIN', 'KOUPREY', 'KRUSAR', 'KESSOR', 'CAESAR', 'SOBEN-P');
...
-- Update the vehicle_model_id for KESSOR vehicles
UPDATE `vehicles` v
JOIN `vehicle_models` vm ON vm.name = 'KAIN'
SET v.vehicle_model_id = vm.id
WHERE v.model = 'KAIN' AND v.vehicle_model_id IS NULL;

-- Then remove the KESSOR model
DELETE FROM `vehicle_models` WHERE `name` = 'KESSOR';

-- Step 10: Add foreign key constraint for vehicles
ALTER TABLE `vehicles`
ADD CONSTRAINT IF NOT EXISTS `fk_vehicles_vehicle_model`
FOREIGN KEY (`vehicle_model_id`) REFERENCES `vehicle_models`(`id`)
ON DELETE SET NULL ON UPDATE CASCADE;

-- Step 11: Show complete vehicle models overview
SELECT
...
-- Step 12: Show price summary by category
SELECT
...
-- Step 13: Show vehicles with model relationships
SELECT
  'VEHICLES WITH MODEL RELATIONSHIPS' as title,
  '' as separator;
