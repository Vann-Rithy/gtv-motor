-- Database Schema Fixes Required for GTV Motor System
-- This script addresses critical issues found in the current database schema

-- ==============================================
-- CRITICAL ISSUES IDENTIFIED:
-- ==============================================
-- 1. Missing PRIMARY KEY constraints on several tables
-- 2. Missing AUTO_INCREMENT settings for primary keys
-- 3. Missing foreign key constraints for core relationships
-- 4. Inconsistent data types and field naming
-- 5. Missing indexes for performance optimization
-- 6. Duplicate/redundant columns in some tables
-- ==============================================

-- Step 1: Fix PRIMARY KEY constraints and AUTO_INCREMENT
-- ==============================================
...
-- Step 4: Fix data inconsistencies and cleanup
-- ==============================================

-- Remove duplicate columns in vehicles table
-- The 'vehicle_plate' column is redundant with 'plate_number'
ALTER TABLE `vehicles` DROP COLUMN IF EXISTS `vehicle_plate`;

-- Remove duplicate columns in customers table
-- These columns are redundant with the main fields
...
-- This column is redundant with the main 'name' field
...
-- Ensure unique VIN numbers (where not null)
-- Note: This will only apply to non-null VIN numbers
CREATE UNIQUE INDEX `unique_vin_number` ON `vehicles`(`vin_number`) WHERE `vin_number` IS NOT NULL;
...
-- Step 6: Fix data type inconsistencies
-- ==============================================
...
ALTER TABLE `warranties` MODIFY `warranty_cost_covered` decimal(10,2) DEFAULT 0.00;
...
-- Step 7: Add missing CHECK constraints for data validation
-- ==============================================
...
-- Step 9: Update AUTO_INCREMENT values to prevent conflicts
-- ==============================================
...
SHOW CREATE TABLE `service_alerts`;
SHOW CREATE TABLE `service_items`;
