-- Safe GTV Motor Vehicle Models Setup
-- This script checks table structure and safely updates vehicle models

-- Step 1: Check current vehicles table structure
SELECT
  'CURRENT VEHICLES TABLE STRUCTURE:' as info,
  column_name,
  data_type,
  is_nullable,
  column_default
...
-- Step 2: Check if vehicle_models table exists
SELECT
  CASE
...
-- Step 3: Create/Update vehicle_models table structure
...
-- Step 4: Insert/Update SOBEN (Compact Entry SUV)
...
-- Step 5: Insert/Update CAESAR (Mid-level SUV)
INSERT INTO `vehicle_models` (
  `name`, `description`, `category`, `base_price`, `estimated_duration`,
  `warranty_km_limit`, `warranty_max_services`, `engine_type`, `cc_displacement`,
  `fuel_type`, `transmission`, `color_options`, `year_range`, `specifications`, `is_active`
...
-- Step 6: Insert/Update KAIN (Premium SUV)
INSERT INTO `vehicle_models` (
  `name`, `description`, `category`, `base_price`, `estimated_duration`,
  `warranty_km_limit`, `warranty_max_services`, `engine_type`, `cc_displacement`,
  `fuel_type`, `transmission`, `color_options`, `year_range`, `specifications`, `is_active`
...
-- Step 7: Insert/Update KRUSAR (Dual-cab Pick-up Truck)
INSERT INTO `vehicle_models` (
  `name`, `description`, `category`, `base_price`, `estimated_duration`,
  `warranty_km_limit`, `warranty_max_services`, `engine_type`, `cc_displacement`,
  `fuel_type`, `transmission`, `color_options`, `year_range`, `specifications`, `is_active`
...
-- Step 8: Insert/Update SOBEN-P (MPV Multi-purpose Vehicle)
INSERT INTO `vehicle_models` (
  `name`, `description`, `category`, `base_price`, `estimated_duration`,
  `warranty_km_limit`, `warranty_max_services`, `engine_type`, `cc_displacement`,
  `fuel_type`, `transmission`, `color_options`, `year_range`, `specifications`, `is_active`
...
-- Step 9: Check if vehicles table has model column
SELECT
  CASE
    WHEN COUNT(*) > 0 THEN 'EXISTS'
    ELSE 'NOT EXISTS'
  END as model_column_status
FROM information_schema.columns
WHERE table_schema = DATABASE()
  AND table_name = 'vehicles'
  AND column_name = 'model';

-- Step 10: Add vehicle_model_id column to vehicles table if it doesn't exist
...
-- Step 11: Show current vehicles data
SELECT
  'CURRENT VEHICLES DATA (first 10 records):' as info,
  id,
  customer_id,
  plate_number,
  model,
...
-- Step 12: Show current vehicle models
SELECT
  'CURRENT VEHICLE MODELS:' as info,
  id,
  name,
  category,
  base_price,
...
-- Step 13: Remove KESSOR model (not in the provided specifications)
DELETE FROM `vehicle_models` WHERE `name` = 'KESSOR';

-- Step 14: Show final vehicle models overview
SELECT
  'FINAL GTV MOTOR VEHICLE MODELS:' as title,
  '' as separator;
